...
def parse_l1000fwd_results(raw: dict, top_n: int = 50) -> list[dict]:
    ...
    for entry in opposite:
        ...
        candidates.append({
            ...
            "clinical_phase": info.get("clinical_phase", ""),
            "confidence":     "high" if qvalue < 0.05
                              else "medium" if qvalue < 0.20
                              else "low"
        })

    # Deduplicate by drug name, keeping the most negative (strongest reversal) score per drug.
    # L1000FWD combined_scores are negative; more negative = stronger reversal.
    seen = {}
    for c in candidates:
        drug = c["drug_name"]
    ...
    return sorted(seen.values(), key=lambda x: x["score"])[:top_n]
